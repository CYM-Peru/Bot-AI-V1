...
export function NodeCard(props: NodeCardProps) {
  ...
  // Ensure at least 48px spacing between handles to prevent label overlap
  const HANDLE_SPACING = 48;
  const BASE_HEIGHT = 180; // Base height for the node content
  const TOP_PADDING = 40; // Space for first handle
  const BOTTOM_PADDING = 40; // Space for last handle
  const minHeight = handleCount > 2
    ? BASE_HEIGHT + (handleCount - 2) * HANDLE_SPACING
    : BASE_HEIGHT;
  ...
  return (
    ...
    >
      ...
      {data.handleSpecs.map((spec, index) => {
        ...
        // Use pixel-based positioning instead of percentage for consistent spacing
        // Distribute handles evenly with guaranteed minimum spacing
        const topPosition = TOP_PADDING + (index * HANDLE_SPACING);
        return (
