...
export function QueueManagement() {
  ...
  const getDistributionLabel = (mode: string) => {
    const labels = {
      "round-robin": "Equitativo (nuevos chats)",
      "least-busy": "Menos Ocupado (nuevos chats)",
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="mt-1 w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                    title="Selecciona cómo se distribuyen los nuevos chats entre asesores"
                  >
                    <option value="least-busy">Menos Ocupado (recomendado)</option>
                    <option value="round-robin">Round Robin (rotación)</option>
                    <option value="manual">Manual (sin auto-asignación)</option>
                  </select>
                  <p className="mt-1 text-xs text-slate-500">
                    {formData.distributionMode === "least-busy" && "✅ Nuevos chats van al asesor con menos carga actual"}
                    {formData.distributionMode === "round-robin" && "🔄 Nuevos chats rotan equitativamente entre asesores"}
                    {formData.distributionMode === "manual" && "👤 Asesores deben aceptar chats manualmente"}
                  </p>
                </div>
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mb-3">Solo asesores con rol "asesor" reciben chats automáticamente</p>
                <div className="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto rounded-lg border border-slate-200 p-3">
                  {advisors.filter(a => a.role === 'asesor').map((advisor) => (
                    <label
                      key={advisor.id}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-3">
                  👀 Supervisores ({formData.supervisors.length})
                </label>
                <p className="text-xs text-slate-500 mb-3">Supervisores solo monitorean, NO reciben chats automáticamente</p>
                <div className="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto rounded-lg border border-purple-200 p-3 bg-purple-50/30">
                  {advisors.filter(a => a.role === 'supervisor' || a.role === 'admin' || a.role === 'gerencia').map((supervisor) => (
                    <label
                      key={supervisor.id}
                      className="flex items-center gap-3 rounded-lg border border-purple-200 bg-white p-3 hover:bg-purple-50 cursor-pointer transition"
                    >
                      <input
                        type="checkbox"
                        checked={formData.supervisors.includes(supervisor.id)}
                        onChange={() => toggleSupervisor(supervisor.id)}
                        className="h-4 w-4 rounded border-slate-300 text-purple-600 focus:ring-purple-500"
                      />
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-purple-100 text-purple-700 text-xs font-semibold">
                        {supervisor.name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2)}
                      </div>
                      <div className="flex-1">
                        <span className="text-sm font-medium text-slate-900 block">{supervisor.name}</span>
                        <span className="text-xs text-purple-600">{supervisor.role}</span>
                      </div>
                    </label>
                  ))}
                </div>
