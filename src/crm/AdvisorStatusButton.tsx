  ...
  const handleChangeStatus = async (status: AdvisorStatus) => {
    try {
      ...
      if (response.ok) {
        ...
        // Emit custom event to notify other components that status changed
        window.dispatchEvent(new CustomEvent('advisor-status-changed', {
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className={compact
          ? "h-7 w-7 rounded-full border-2 border-white shadow-md transition hover:scale-110 hover:shadow-lg flex items-center justify-center"
      ...
      {showMenu && (
        <div className="absolute top-full right-0 mt-2 w-64 rounded-xl border border-slate-200 bg-white shadow-2xl z-[9999]">
