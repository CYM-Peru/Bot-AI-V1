...
export default function Composer({ disabled, onSend, replyingTo, onCancelReply }: ComposerProps) {
  ...
  const handleSend = async () => {
    ...
    console.log('[Composer] 📤 handleSend called - text:', text.trim(), 'files:', files.length);
    setSending(true);
    try {
      console.log('[Composer] 📤 Calling onSend with payload:', {
        text: text.trim(),
      ...
      console.log('[Composer] ✅ onSend completed successfully');
      setText("");
      setFiles([]);
      setIsInternal(false);
      ...
      console.error('[Composer] ⚠️ ERROR en handleSend:', error);
      // Re-throw para que el componente padre lo maneje
      throw error;
    } finally {
      setSending(false);
    }
  ...
  const handlePaste = (event: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      // Si es un archivo (imagen, documento, etc.)
      if (item.kind === 'file') {
    ...
    // Si encontramos archivos, agregarlos y prevenir pegado de texto
    if (pastedFiles.length > 0) {
      event.preventDefault(); // Prevenir el pegado default solo si hay archivos

      const newFiles = [...files, ...pastedFiles];
      setFiles(newFiles);

      // Crear URLs de preview para los nuevos archivos
      const newUrls = pastedFiles.map(file => URL.createObjectURL(file));
      setPreviewUrls([...previewUrls, ...newUrls]);

      console.log(`[Composer] ${pastedFiles.length} archivo(s) pegado(s)`);
    }
    // Si solo hay texto, dejar que el textarea lo maneje normalmente (no hacer nada)
  };
            ...
            {files.map((file, index) => (
              <div key={index} className="relative group">
                <button
                  ...
                  className="absolute -top-2 -right-2 z-[9000] bg-rose-500 hover:bg-rose-600 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition shadow-lg"
                >
        ...
        <div className="flex items-end gap-3 relative">
          ...
          {showAttachmentMenu && (
            <>
              ...
              />
              <div className="absolute bottom-14 left-0 z-[9999] w-64 bg-white rounded-2xl shadow-2xl border-2 border-slate-200 overflow-hidden">
