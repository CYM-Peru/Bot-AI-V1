...
export default function BitrixContactCard({ conversation }: BitrixContactCardProps) {
  ...
  const handleCreateContact = async () => {
    ...
    setLoading(true);
    try {
      const response = await authFetch(`/api/crm/conversations/${conversation.id}/bitrix/create`, {
        method: "POST",
  ...
  useEffect(() => {
    ...
    const load = async (retryCount = 0) => {
      ...
      setStatus(null);
      try {
        const response = await authFetch(`/api/crm/conversations/${conversation.id}/bitrix`);
        if (!response.ok) {
