...
export function getWhatsAppEnv(): WhatsAppEnvConfig {
  ...
  const phoneNumberId = readEnv("WSP_PHONE_NUMBER_ID") ?? readEnv("WHATSAPP_PHONE_NUMBER_ID");
  const accessToken = readEnv("WSP_ACCESS_TOKEN") ?? readEnv("WHATSAPP_ACCESS_TOKEN");
  const verifyToken = readEnv("WSP_VERIFY_TOKEN") ?? readEnv("WHATSAPP_VERIFY_TOKEN");

  return { baseUrl, apiVersion, phoneNumberId, accessToken, verifyToken };
}
...
const BITRIX_TOKEN_PATH = path.resolve(process.cwd(), "data", "bitrix-tokens.json");
