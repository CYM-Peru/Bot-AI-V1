...
export class CrmRealtimeManager {
  ...
  private async acceptConnection(req: IncomingMessage, socket: NetSocket, head: Buffer, key: string) {
    try {
    ...
    console.log(`[CRM Socket] ✅ NEW client connected - ID: ${id.substring(0, 8)}... - Total clients: ${this.clients.size}`);
  ...
  private async handleSocketData(client: ClientConnection, chunk: Buffer) {
  ...
  private async handleClientMessage(client: ClientConnection, payload: Buffer) {
    try {
  ...
  private async handleClientEvent(client: ClientConnection, event: string, data: unknown) {
     switch(event) {
      case SOCKET_EVENTS.read: {
         if(typeof data === "string") {
          await crmDb.markConversationRead(data);
          const conversation = await crmDb.getConversationById(data);
          if (conversation) {
  ...
  private broadcast(event: string, payload: unknown, excludeId?: ClientId) {
    console.log(`[CRM Socket] 📡 broadcast() called - event: ${event}, clients: ${this.clients.size}`);

    if (this.clients.size === 0) {
      console.warn(`[CRM Socket] ⚠️ NO clients connected - event will NOT be delivered!`);
      return;
    }
    ...
    for (const [id, client] of this.clients.entries()) {
      ...
      try {
        ...
        sentCount++;
        console.log(`[CRM Socket] ✅ Event sent to client ${id.substring(0, 8)}...`);
      } catch (error) {
        errorCount++;
    ...
    console.log(`[CRM Socket] 📊 Broadcast complete - sent: ${sentCount}, errors: ${errorCount}`);
  }
  ...
  private dropClient(clientId: ClientId) {
    ...
    console.log(`[CRM Socket] ❌ Client disconnected - ID: ${clientId.substring(0, 8)}... - Remaining clients: ${this.clients.size}`);
    try {
